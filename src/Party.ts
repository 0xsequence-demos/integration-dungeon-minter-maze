...
import { Direction, RelativeDir, asRadians, deltaX, deltaY, relativeTo, rotatedCCW, rotatedCW } from "./directionUtils";
...
export class Party {
    ...
    keyActions = {
        move_forward: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.FORWARD);
        }},
        move_right: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.RIGHT);
        }},
            move_left: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.LEFT);
        }},
        move_backward: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.BACKWARD);
        }},
        rotate_left: {movement: true, action: ()=> {
            this.rotateCCW();
        }},
        rotate_right: {movement: true, action: ()=> {
            this.rotateCW();
        }},

        move_forward_arrows: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.FORWARD);
        }},
        move_right_arrows: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.RIGHT);
        }},
        move_left_arrows: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.LEFT);
        }},
        move_backward_arrows: {movement: true, action: ()=> {
            this.moveRelative(RelativeDir.BACKWARD);
        }},
        rotate_left_arrows: {movement: true, action: ()=> {
            this.rotateCCW();
        }},
        rotate_right_arrows: {movement: true, action: ()=> {
            this.rotateCW();
        }}
    };
    light: PointLight;
    constructor(private camera:PerspectiveCamera, x:number, y:number, dir:Direction, collisions:(x:number, y:number) => boolean) {
        ...
        this.light = new PointLight(PARTY.LIGHT.COLOR, PARTY.LIGHT.INTENSITY, PARTY.LIGHT.RADIUS);
        ...
        move(dir) {
            ...
            var newX = this.position.x + deltaX(dir);
            var newY = this.position.y + deltaY(dir);
        ...
        rotateTo(dir) {
            ...
            var rotateAmount = asRadians(dir) - mod(this.camera.rotation.y, 2*Math.PI);
            rotateAmount = mod(rotateAmount + Math.PI, 2*Math.PI) - Math.PI

            new Tween(this.camera.rotation)
                .to({y: self.camera.rotation.y + rotateAmount}, PARTY.ROTATE_TIME)
                .easing(Easing.Sinusoidal.InOut)
